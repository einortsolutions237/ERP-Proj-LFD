...
- Per this project's architecture ("Firebase custom claims are the sole source of truth for authorization; Firestore is profile metadata only and is never read to make an access-control decision" — CLAUDE.md, Permissions section): the **sender's** role/branchId comes from their verified session cookie (already claims-backed). The **recipient's** role/branchId for the `canMessage` check must be re-fetched live from Firebase Auth custom claims (`getAdminAuth().getUser(uid).customClaims`), never from the `staff` Firestore doc — the `staff` doc is only ever used here for display metadata (name), never for the authorization decision itself.
