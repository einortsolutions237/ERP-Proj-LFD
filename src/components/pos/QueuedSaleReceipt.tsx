    ...
    <div className="mx-auto max-w-md space-y-4 rounded-2xl border border-mist bg-surface p-6 shadow-[var(--shadow-card)]">
      ...
      <div className="space-y-1 text-sm">
        {receipt.lineItems.map((line, i) => (
          <div key={i} className="flex justify-between">
            ...
            <span className="font-mono text-ink text-right">{line.lineTotal.toFixed(2)}</span>
          </div>
        ))}
      </div>
      <div className="space-y-1 border-t border-mist pt-2 text-sm">
        <div className="flex justify-between">
          ...
          <span className="font-mono text-ink text-right">{receipt.subtotal.toFixed(2)}</span>
        </div>
        <div className="flex justify-between font-semibold">
          <span className="text-ink">Total</span>
          <span className="font-mono text-ink text-right">{receipt.total.toFixed(2)}</span>
        </div>
      </div>
